...
class ErrorManager: ErrorManagerProtocol {
    ...
    private func receive(_ state: AppState) {
        ...
        updateErrorEventHandler(state.errorState)
        updateEvictedEventHandler(state.errorState)
        updateFatalError(state.errorState)
    }

    private func updateErrorEventHandler(_ errorState: ErrorState) {
    ...
    private func updateEvictedEventHandler(_ errorState: ErrorState) {
        guard let didEvictLocalParticipant = eventsHandler.onLocalParticipantRemoved,
              errorState.internalError == .chatEvicted else {
            return
        }
        didEvictLocalParticipant()
    }
