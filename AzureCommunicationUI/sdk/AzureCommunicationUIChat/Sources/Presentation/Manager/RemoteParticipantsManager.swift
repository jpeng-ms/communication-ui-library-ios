...
class RemoteParticipantsManager: RemoteParticipantsManagerProtocol {
    ...
    private func receive(_ state: AppState) {
        ...
        let addedIdentifiers = joinedParticipantsIds.compactMap { id in
            state.participantsState.participantsInfoMap[id]?.identifier
        }

        postRemoteParticipantsJoinedEvent(addedIdentifiers)
    ...
    private func postRemoteParticipantsJoinedEvent(_ joinedParticipantsIds: [CommunicationIdentifier]) {
        ...
        didRemoteParticipantsJoin(joinedParticipantsIds)
    }
}
