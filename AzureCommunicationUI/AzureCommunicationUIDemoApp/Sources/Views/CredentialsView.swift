...
extension CredentialsView {
    ...
    private func subscribeEvents() {
        let onParticipantsJoinedHandler: ([CommunicationIdentifier]) -> Void = { participantsId in
            print("---Participants joined \(participantsId)")
        }
        chatComposite.events.onRemoteParticipantJoined = onParticipantsJoinedHandler

        let onNavigateBackHandler: (() -> Void) = {
            print("---Navigate back")
        }
        chatComposite.events.onNavigateBack = onNavigateBackHandler

        let onNewMessageReceivedHandler: (ChatMessageModel) -> Void = { message in
            print("---Event handler message: \(message.content)")
        }
        chatComposite.events.onNewMessageReceived = onNewMessageReceivedHandler

        let onNewUnreadMessageReceivedHandler: (Int) -> Void = { unreadCount in
            print("---Event handler unread messages: \(unreadCount) count")
            unreadMessageCount = unreadCount
        }
        chatComposite.events.onNewUnreadMessages = onNewUnreadMessageReceivedHandler

        let onLocalParticipantRemovedHandler: (() -> Void) = {
            print("---Local user has been removed from chat thread")
        }
        chatComposite.events.onLocalParticipantRemoved = onLocalParticipantRemovedHandler
    }
}
